...
def benchmark(
    torch_spec, triton_ops: Dict[str, Callable], nelem={}, B={"B0": 32}, device="gpu"
) -> bool:
    """
    ...
    for name, triton_op in triton_ops.items():
        ...
        print(
            f"Time: {triton_time * 1000:.4f} ms ({torch_time / triton_time:.2f}x speedup)"
        )
        ...
        if not match:
            ...
            if mask.any():
                ...
                for i in range(sample_size):
                    ...
                    print(
                        f"  At {idx}: yours={z[idx].item():.6f}, ref={z_ref[idx].item():.6f}, diff={z[idx].item() - z_ref[idx].item():.6f}"
                    )
...
if __name__ == "__main__":
    ...
    flash_att_ops = {
        "two_pass": two_pass_flashatt_kernel,
        "one_pass": one_pass_flashatt_kernel,
    }
